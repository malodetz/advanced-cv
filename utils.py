...
def non_max_suppression(predictions, conf_thresh=0.25, iou_thresh=0.5):
    # Convert predictions to [batch, boxes, (x1, y1, x2, y2, conf, class)]
    all_boxes = []
...
def mean_average_precision(pred_boxes, true_boxes, iou_threshold=0.5):
    ...
    for c in range(Config.num_classes):
        ...
        # Initialize TP/FP lists and GT tracking
        tp = []
        fp = []
        total_gt = sum(len(gts) for gts in ground_truths.values())
        
        matched_gts = {img_idx: [False]*len(gts) for img_idx, gts in ground_truths.items()}
        
        # Calculate TP/FP for each detection
