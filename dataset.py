    ...
    def __getitem__(self, idx):
        ...
        with open(self.labels[idx], 'r') as f:
            for line in f.readlines():
                ...
                target[grid_y, grid_x, :5*self.num_boxes] = torch.cat([box_data]*self.num_boxes)
...
def create_dataloaders():
    # Get all files
    images = sorted([os.path.join(Config.data_dir, f) 
                    for f in os.listdir(Config.data_dir) if f.endswith(".jpg")])
