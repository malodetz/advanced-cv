...
def extract_frames(video_path, output_dir, frames_per_second=1):
    ...
    while True:
        ...
        if current_frame % step == 0:
            frame_path = os.path.join(output_dir, f"{video_name}_frame_{frame_number:05d}.jpg")
...
def extract_all_frames(input_dir, output_dir, frames_per_second=10):
    ...
    video_files = [f for f in os.listdir(input_dir) if f.endswith('.mkv') and f.startswith('Movie_')]
    total_frames = 0
